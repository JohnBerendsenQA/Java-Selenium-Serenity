...
public class BrowserStackSerenityDriver implements DriverSource  {
    ...
    public WebDriver newDriver() {
        ...
        while(it.hasNext()){
            ...
            else if(environment != null && key.startsWith("browser." + environment)){
                
                String builddate = new SimpleDateFormat("ddMMyyyy").format(new Date());
                capabilities.setCapability("build", builddate + " # Smoketest");
                capabilities.setCapability(key.replace("browser." + environment + ".", ""), environmentVariables.getProperty(key));
                if(key.equals("browser." + environment + ".browserstack.local")){
                    System.setProperty("browserstack.local", "true");
                }
            }
        }
